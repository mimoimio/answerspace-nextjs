...
export default async function Post({ params }: { params: Promise<{ post_id: string }> }) {
    ...
    const post = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/posts`, { cache: "no-store" }).then((res) => res.json()).then((data) => data.posts.find((post: any) => post.post_id === parseInt(post_id)))
